...
const ProjectList = () => {
    ...
    const deleteProject = async (projectId) => {
        try {
            await axios.delete(`http://localhost:3001/project/delete/${projectId}`);
            fetchProjects();
        } catch (error) {
...
const ProjectForm = ({ onSubmit }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="project-form">
            ...
            <input id="dateD" name="dateD" type='Date' value={formData.dateD} onChange={handleChange} required />

            <label htmlFor="dateF">Date fin :</label>
            <input id="dateF" name="dateF" type='Date' value={formData.dateF} onChange={handleChange} required />
